...
export default function EditAttendancePage() {
  ...
  return <div className="min-h-screen"><Sidebar /><div className="lg:pl-[272px]"><Navbar /><main className="max-w-5xl d7-page"><Link href="/attendance" className="mb-6 inline-flex items-center gap-2 text-sm text-slate-500"><ArrowLeft size={16} />Back to attendance</Link><div className="mb-8"><p className="mb-2 text-sm font-semibold text-violet-700">Attendance record</p><h1 className="text-3xl font-semibold text-slate-900">Edit attendance</h1><p className="mt-2 text-sm text-slate-500">Update status, score, date, or remarks.</p></div>{error ? <p className="rounded-lg bg-red-50 p-4 text-sm text-red-700">{error}</p> : !record ? <p className="text-sm text-slate-500">Loading attendance...</p> : <AttendanceForm students={students} initialValue={initialValue} onSubmit={submit} />}</main></div></div>;
}