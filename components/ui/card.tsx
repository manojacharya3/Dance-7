...
export function StatCard({
  ...
  tone?: "violet" | "emerald" | "amber" | "blue" | "rose" | "slate";
}) {
  ...
  return (
    <div className="d7-card d7-card-hover">
      ...
      {sub ? <p className="mt-1 text-[13px] leading-5 text-slate-400">{sub}</p> : null}
    </div>
  );
}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center px-6 py-14 text-center">
      {icon ? (
        <span className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-50 text-violet-600">
          {icon}
        </span>
      ) : null}
